...
class MeetingPanel:
    def __init__(self, link, agents: list) -> None:
        ...
        self.voice_on = True          # speak the meeting aloud (Gemini TTS)
        self.player = None            # VoicePlayer, created when a voiced meeting starts
    ...
    def _start(self) -> None:
        ...
        if len(ids) < 2 or not self.topic.strip():
            return
        self.link.start(self.topic.strip(), ids, mode="moderated")
        self.mode = "live"
        self._lines = []
        self._scroll = 0
        if self.voice_on and self.player is None:
    ...
    def update(self) -> None:
        ...
        if self.mode == "invite":
            ...
            for name, content in self.link.poll():
                ...
                if self.voice_on and self.player is not None:
    ...
    def _draw_invite(self, x: int, y: int) -> None:
        ...
        # Voice toggle
        vrow = pr.Rectangle(x + PAD, y + PANEL_H - 96, 380, 26)
        vbox = pr.Rectangle(vrow.x, vrow.y, 18, 18)
        pr.draw_rectangle_lines_ex(vbox, 2, GOOD if self.voice_on else MUTED)
        if self.voice_on:
            pr.draw_rectangle(int(vbox.x) + 4, int(vbox.y) + 4, 10, 10, GOOD)
        pr.draw_text("Speak the meeting aloud (Gemini voices)", int(vbox.x) + 28, int(vbox.y) + 1, 16, pr.RAYWHITE)
        if pr.is_mouse_button_pressed(pr.MOUSE_BUTTON_LEFT) and pr.check_collision_point_rec(mouse, vrow):
        ...
        start = pr.Rectangle(x + PANEL_W - PAD - 230, y + PANEL_H - 56, 230, 40)
        if _btn(cancel, "Cancel", pr.Color(120, 60, 60, 255)):
            self.close()
        if _btn(start, f"Start Meeting ({n})", pr.Color(80, 120, 70, 255), enabled=can_start) and can_start:
            self._start()

    def _draw_live(self, x: int, y: int) -> None:
        title = f"Meeting: {self.link.topic}"
        ...
        vlabel = "Voice: ON" if self.voice_on else "Voice: OFF"
        vcol = pr.Color(80, 120, 70, 255) if self.voice_on else pr.Color(90, 70, 70, 255)
    ...
    def _toggle_voice(self) -> None:
        self.voice_on = not self.voice_on
        if not self.voice_on and self.player is not None:
            self.player.clear()                 # stop queued speech immediately
        elif self.voice_on and self.player is None:
            self.player = VoicePlayer()
